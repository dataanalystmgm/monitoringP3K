...
const App = () => {
  ...
  const [activeTab, setActiveTab] = useState('dashboard'); // Menu: dashboard atau calendar
  const [currentDate, setCurrentDate] = useState(new Date()); // Navigasi bulan kalender
...
//test-push
  ...
  const getStationStatusByDate = (id, date) => {
    ...
    const logs = submissions.filter(s => 
      s.stationId === id && 
      new Date(s.timestamp).toLocaleDateString('en-CA') === targetDateStr
    );
  ...
  const getDayAnalytics = (date) => {
    ...
    // Prioritas Warna: Merah (Not Start) > Orange (Uncomplete) > Kuning (Unvalidated) > Hijau (Clear)
    let bgColor = 'cal-green';
    let displayIssues = [];

    if (issues.red.length > 0) {
      bgColor = 'cal-red';
      displayIssues = issues.red;
    } else if (issues.orange.length > 0) {
      bgColor = 'cal-orange';
      displayIssues = issues.orange;
    } else if (issues.yellow.length > 0) {
      bgColor = 'cal-yellow';
      displayIssues = issues.yellow;
    }
          ...
          {loading && submissions.length === 0 ? (
            ...
            <div className="grid-layout-8col">
              {[...Array(40)].map((_, i) => {
                ...
                return (
                  <StationCard 
                    key={id} id={id} status={status.color} 
                    isValidated={status.validated} onClick={() => handleCardClick(id)} 
                  />
                );
              })}
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content animate-pop">
            ...
            <div className="modal-footer">
              ...
              {selectedStation.data.isComplete && !selectedStation.validated && (
                <button className="btn-modern btn-validate" onClick={handleValidate}>Validate Now</button>
              )}
              {selectedStation.validated && <div className="badge-validated">Validated ✓</div>}
            </div>
          </div>
        </div>
      )}
